...
Prof_TimerRecord prof_debug_records_array[];
// NOTE(christian): debug stuff
fun void
prof_start_profile_function(u32 counter_init, char *function_name, char *file_name,
                            u32 line_number) {
  Prof_TimerRecord *timer_record = prof_debug_records_array + counter_init;
  timer_record->time_value = intrin_rdtsc();
  timer_record->file_name = file_name;
  timer_record->function_name = function_name;
  timer_record->counter_index = counter_init;
  timer_record->line_number = line_number;
}

fun void
prof_end_profile_function(u32 counter_init) {
  Prof_TimerRecord *timer_record = prof_debug_records_array + counter_init;
  timer_record->time_value = intrin_rdtsc() - timer_record->time_value;
}
